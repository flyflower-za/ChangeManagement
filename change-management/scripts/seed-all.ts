...
async function main() {
  ...
  // ========== 测试变更项目 ==========
  console.log('\n9. 创建测试变更项目...')
  const change1 = await createChange('产品A产线升级改造', '对产品A相关产线进行升级', 'high', admin.id, [productA.id], allModules, lisi.id)
  console.log(`   ✓ ${change1.title}`)
  const change2 = await createChange('产品B质量优化项目', '优化产品B的生产质量流程', 'medium', admin.id, [productB.id], allModules, lisi.id)
...
async function createChange(title: string, desc: string, priority: string, creatorId: string, productIds: string[], allModules: any[], executorId: string) {
  ...
  const change = await prisma.changeProject.create({
    data: { title, description: desc, priority, status: 'EXECUTING', plannedStart: new Date(Date.now() + 86400000), plannedEnd: new Date(Date.now() + 3 * 86400000), createdById: creatorId }
  })
